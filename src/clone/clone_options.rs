...
/// --local, -l
pub fn local() -> CommandOption<'static> {
...
/// --dissociate
pub fn dissociate() -> CommandOption<'static> {
...
/// --no-checkout, -n
pub fn no_checkout() -> CommandOption<'static> {
...
/// --bare
pub fn bare() -> CommandOption<'static> {
...
/// --origin <name>, -o <name>
pub fn origin(name_arg :&str) -> CommandOption {
...
/// --branch <name>, -b <name>
pub fn branch(name_arg :&str) -> CommandOption {
    Box::new(move |g: &mut CommandExecutor| {
        g.add_option("--branch");
        g.add_option(name_arg);
    })
...
pub fn template(template_directory_arg :&str) -> CommandOption {
...
/// --single-branch
pub fn single_branch() -> CommandOption<'static> {
    Box::new(|g: &mut CommandExecutor| g.add_option("--single-branch"))
}

/// Clone only the history leading to the tip of a single branch, either specified by the --branch option or the primary branch remote’s HEAD points at. Further fetches into the resulting repository will only update the remote-tracking branch for the branch this option was used for the initial cloning. If the HEAD at the remote did not point at any branch when --single-branch clone was made, no remote-tracking branch is created.
/// --no-single-branch
pub fn no_single_branch() -> CommandOption<'static> {
