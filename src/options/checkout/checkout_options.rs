...
/// --ours, --theirs
pub fn ours() -> CommandOption<'static> {
    Box::new(|g: &mut CommandExecutor| g.add_option("--ours"))
}

/// When checking out paths from the index, check out stage #2 (ours) or #3 (theirs) for unmerged paths.
/// --ours, --theirs
pub fn theirs() -> CommandOption<'static> {
...
/// -b [new_branch]
pub fn new_branch(new_branch_arg :&str) -> CommandOption {
...
/// -B [new_branch]
pub fn new_branch_force(new_branch_arg :&str) -> CommandOption {
    Box::new(move |g: &mut CommandExecutor| {
         g.add_option("-B");
        if !new_branch_arg.is_empty() {
            g.add_option(new_branch_arg);
...
/// --no-track
pub fn no_track() -> CommandOption<'static> {
    Box::new(|g: &mut CommandExecutor| g.add_option("--no-track"))
}

/// Create the new branch’s reflog; see git-branch(1) for details.
/// -l
pub fn new_branch_reflog() -> CommandOption<'static> {
...
/// --detach
pub fn detach() -> CommandOption<'static> {
...
/// --orphan <new_branch>
pub fn orphan(new_branch_arg :&str) -> CommandOption {
    Box::new(move |g: &mut CommandExecutor| {
        g.add_option("--orphan");
        g.add_option(new_branch_arg);
    })
...
pub fn ignore_skip_worktree_bits() -> CommandOption<'static> {
...
/// --conflict=<style>
pub fn conflict(style_arg :&str) -> CommandOption {
...
pub fn ignore_other_worktrees() -> CommandOption<'static> {
